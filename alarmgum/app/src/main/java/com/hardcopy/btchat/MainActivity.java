...
public class MainActivity extends FragmentActivity implements ActionBar.TabListener, IFragmentListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		boolean isRunIntro;
		isRunIntro = getIntent().getBooleanExtra("intro", false);
		if(isRunIntro==false) {
			beforeIntro();
		}
		...
		public void handleMessage(Message msg) 
		{
			...
			case Constants.MESSAGE_READ_CHAT_DATA:
				...
				if(msg.obj != null) {
					...
					Logs.d(TAG, "strA's value is" + strA);
					...
					currentCursor2 = idb.fetchAllAlarm();
					Cursor ic = idb.fetchAllAlarm();
					if (c.moveToFirst())
					{
						do {

							if(strA.equals(c.getString(c.getColumnIndex(DBAdapter.DEVICE_NAME))))
							{
								if(c.getInt(c.getColumnIndex(DBAdapter.ALARM_ON))==1)
								{
									vib=c.getInt(c.getColumnIndex(DBAdapter.ALARM_VIBRATE));
									ring =c.getString(c.getColumnIndex(DBAdapter.ALARM_RINGTONE));
									proname =c.getString(c.getColumnIndex(DBAdapter.PRODUCT_NAME));
									devname =c.getString(c.getColumnIndex(DBAdapter.DEVICE_NAME));
										Logs.d(TAG, "this device get string alramon so start testalarm activity...");
					...
					if(flag==true)
					{
						...
						Intent intent=new Intent(MainActivity.this,testalrampage.class);
						intent.putExtra("inputval",(String)msg.obj);
						intent.putExtra("vibeon",vib);
						intent.putExtra("ringtone",ring);
				...
				//원래 close가 있던자리
					...
					frg.showMessage((String)msg.obj +" "+strNow);
